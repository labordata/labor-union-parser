"""ArcFace F_num Classifier with FastText Encoder + Digit Encoding.

Extends arcface_fasttext_spike.py: number tokens (is_num=True) use positional
digit embeddings matching the prototype side, instead of vocab+n-gram lookup.
This way the encoder and prototype speak the same language about numbers.
...
DIGIT_LEN = 8  # max digits per number token (matches prototype side)
...
def encode_digits(token):
    """Encode a number token as reversed digit indices (1-10, 0=pad).

    Same scheme as prototype desig_num encoding: position 0 = ones digit,
    position 1 = tens, etc. Each digit char maps to int(c)+1 so 0=pad.
    """
    digits = [0] * DIGIT_LEN
    # Strip leading zeros
    s = token.lstrip("0") or "0"
    for j, c in enumerate(reversed(s[:DIGIT_LEN])):
        if c.isdigit():
            digits[j] = int(c) + 1
    return digits


def precompute_digit_ids(tokens, is_num):
    """Precompute digit encodings for all tokens. Non-number tokens get all zeros."""
    all_digits = []
    for tok, is_n in zip(tokens, is_num):
        if is_n and tok:
            all_digits.append(encode_digits(tok))
        else:
            all_digits.append([0] * DIGIT_LEN)
    return all_digits
...
class EncoderDigitEmbedding(nn.Module):
    """Positional digit embedding for number tokens (matches prototype side)."""

    def __init__(self, d_model, digit_len=8, digit_dim=16):
        super().__init__()
        self.digit_embed = nn.Embedding(11, digit_dim)  # 0=pad, 1-10=digits
        self.proj = nn.Linear(digit_len * digit_dim, d_model)
        nn.init.normal_(self.digit_embed.weight, std=0.01)

    def forward(self, digit_ids):
        """
        Args:
            digit_ids: (B, L, DIGIT_LEN) int tensor

        Returns:
            (B, L, d_model)
        """
        emb = self.digit_embed(digit_ids)  # (B, L, DIGIT_LEN, digit_dim)
        flat = emb.view(emb.shape[0], emb.shape[1], -1)  # (B, L, DIGIT_LEN * digit_dim)
...
class FastTextRoPEEncoder(nn.Module):
    def __init__(
        ...
        self.digit_embed = EncoderDigitEmbedding(d_model)
    ...
    def forward(self, token_ids, ngram_ids, ngram_counts, digit_ids, is_num, lengths):
        ...
        Args:
            ...
            digit_ids: (B, L, DIGIT_LEN) int — digit encodings for number tokens
            is_num: (B, L) float
            lengths: (B,) int
        ...
        # Digit embedding for number tokens
        num_emb = self.digit_embed(digit_ids)

        # Use digit embedding where is_num, FastText elsewhere
...
class FactoredArcFaceClassifier(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("desig_nums", fnum_desig_nums)
    ...
    def _prototypes(self):
        ...
        digits = self.digit_embed(self.desig_nums)
        dnum = self.dnum_proj(digits.view(digits.size(0), -1))
...
class ArcFaceFastTextModel(nn.Module):
    ...
    def encode(self, token_ids, ngram_ids, ngram_counts, digit_ids, is_num, lengths):
        """Encode to L2-normalized embeddings via masked mean-pool."""
        h = self.encoder(token_ids, ngram_ids, ngram_counts, digit_ids, is_num, lengths)
    ...
    def forward(
        ...
        embeddings = self.encode(
            token_ids, ngram_ids, ngram_counts, digit_ids, is_num, lengths
        )
...
def load_data(path, n_buckets, synthetic_path=None):
    ...
    for ex in raw:
        ...
        digit_ids = precompute_digit_ids(tokens, is_num)
...
def collate_batch(batch, device):
    ...
    digit_ids = torch.zeros(B, max_len, DIGIT_LEN, dtype=torch.long)
    ...
    for i, ex in enumerate(batch):
        ...
        digit_ids[i, :L] = torch.tensor(ex["digit_ids"][:L], dtype=torch.long)
...
def evaluate(model, data, fnum_freq, device, batch_size=512):
    ...
    with torch.no_grad():
        for start in range(0, len(data), batch_size):
            ...
            logits, _ = model(
                token_ids, ngram_ids, ngram_counts, digit_ids, is_num_t, lengths
            )
...
def main():
    ...
    if args.save_checkpoint:
        checkpoint = {
            ...
            "desig_nums": factored_info["desig_nums"],
        }
