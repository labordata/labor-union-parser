"""ArcFace F_num Classifier with FastText + Bloom + FiLM Prototype Factoring.

Same encoder as arcface_fasttext_bloom_spike.py (FastText words + bloom numbers).
Prototype factoring uses FiLM conditioning: union_name modulates all other fields.
Each other field embedding is scaled and shifted by learned functions of the
union_name embedding, so "LU" means different things for different unions.

    modulated_field = gamma(W_union) * W_field + beta(W_union)
    prototype = W_union + sum(modulated_fields) + W_fnum
"""
...
class FiLMConditioner(nn.Module):
    """Produces per-dimension scale (gamma) and shift (beta) from a conditioning vector."""

    def __init__(self, d_model):
        super().__init__()
        self.gamma = nn.Linear(d_model, d_model)
        self.beta = nn.Linear(d_model, d_model)
        # Init gamma to output ones, beta to output zeros — starts as additive
        nn.init.zeros_(self.gamma.weight)
        nn.init.ones_(self.gamma.bias)
        nn.init.zeros_(self.beta.weight)
        nn.init.zeros_(self.beta.bias)

    def forward(self, condition, field_emb):
        """
        Args:
            condition: (N, d_model) — conditioning embedding (union_name)
            field_emb: (N, d_model) — field embedding to modulate

        Returns:
            (N, d_model) — modulated field embedding
        """
        return self.gamma(condition) * field_emb + self.beta(condition)


class FiLMArcFaceClassifier(nn.Module):
    """ArcFace with FiLM-factored prototypes.

    Union_name conditions all other fields via learned scale+shift:
        prototype = W_union + FiLM_dn(W_union, W_desig_name) + FiLM_dnum(W_union, W_dnum) + ... + W_fnum
    """
    ...
    def __init__(
        ...
        # Bloom hash embedding for desig_num
        self.bloom_embed = BloomNumberEmbedding(d_model)

        # FiLM conditioners: union_name modulates each other field
        self.film_desig_name = FiLMConditioner(d_model)
        self.film_dnum = FiLMConditioner(d_model)
        self.film_prefix = FiLMConditioner(d_model)
        self.film_suffix = FiLMConditioner(d_model)
    ...
    def _prototypes(self):
        ...
        # Union_name conditions all other fields
        dn = self.film_desig_name(u, dn)
        dnum = self.film_dnum(u, dnum)
        pfx = self.film_prefix(u, pfx)
        sfx = self.film_suffix(u, sfx)
...
class ArcFaceFastTextModel(nn.Module):
    def __init__(
        ...
        self.arcface = FiLMArcFaceClassifier(
            d_model,
            n_classes,
