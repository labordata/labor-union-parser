...
class ArcFaceFastTextModel(nn.Module):
    def __init__(
        ...
        # Auxiliary union_name classification head
        # Shares weights with prototype W_union — gradient flows into same embeddings
        self.union_head_shared = True  # flag for forward
        self.union_scale = nn.Parameter(torch.tensor(10.0))  # learned temperature
    ...
    def forward(
        ...
        # Auxiliary field classification heads — shared weights with prototypes
        field_losses = {}
        field_weights = {
            "union_name": (self.arcface.W_union.weight[1:], self.union_scale),
            "desig_name": (self.arcface.W_desig_name.weight[1:], self.union_scale),
            "prefix": (self.arcface.W_prefix.weight[1:], self.union_scale),
            "suffix": (self.arcface.W_suffix.weight[1:], self.union_scale),
        }
        if field_targets is not None:
            for field, (W, scale) in field_weights.items():
                ft = field_targets.get(field)
                if ft is not None:
                    valid = ft >= 0
                    if valid.any():
                        field_logits = scale * F.linear(
                            embeddings[valid], F.normalize(W, dim=1)
                        )
                        field_losses[field] = F.cross_entropy(field_logits, ft[valid])

        return logits, arcface_loss, field_losses
...
def main():
    ...
    # Build aux head vocabs: 0-indexed (matching W_field[1:] skip)
    field_vocabs_aux = {
        "union_name": union_vocab,  # already 0-indexed
        "desig_name": {
            v: i
            for i, v in enumerate(sorted(field_vocabs["desig_name"].keys(), key=str))
        },
        "prefix": {
            v: i for i, v in enumerate(sorted(field_vocabs["prefix"].keys(), key=str))
        },
        "suffix": {
            v: i for i, v in enumerate(sorted(field_vocabs["suffix"].keys(), key=str))
        },
    }
    ...
    for epoch in range(args.epochs):
        ...
        for start in range(0, n_train, args.batch_size):
            ...
            _, arcface_loss, field_losses = model(
                token_ids,
                ngram_ids,
                ngram_counts,
                bloom_ids,
