"""ArcFace with FiLM-modulated desig_num + Multi-Prototype.

Based on arcface_fasttext_bloom_multiproto_spike.py. The desig_num bloom
embedding is modulated by the union_name embedding via FiLM, so bloom("31")
points in different directions for CWA vs Teamsters. Other fields stay additive.
...
class MultiProtoArcFaceClassifier(nn.Module):
    ...
    def __init__(
        ...
        # FiLM: union modulates desig_num only
        # gamma(union) = W_gamma @ W_union[i] + b_gamma
        # Init: W_gamma = I, b_gamma = 1 → gamma(u) = u + 1
        # This gives union-dependent modulation from step 0:
        # modulated_dnum = (W_union[i] + 1) * bloom(dnum) + beta
        self.film_gamma = nn.Linear(d_model, d_model)
        self.film_beta = nn.Linear(d_model, d_model)
        nn.init.eye_(self.film_gamma.weight)
    ...
    def _prototypes(self):
        ...
        # FiLM: union modulates desig_num
        gamma = self.film_gamma(u)
        beta = self.film_beta(u)
        dnum = gamma * dnum + beta
...
def main():
    ...
    # Separate param groups: higher LR for FiLM params
    film_params = list(model.arcface.film_gamma.parameters()) + list(
        model.arcface.film_beta.parameters()
    )
    ...
    optimizer = torch.optim.AdamW(
        [
            ...
            {"params": film_params, "lr": args.lr * 10},
        ]
    )
