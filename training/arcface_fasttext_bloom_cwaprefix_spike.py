...
def main():
    ...
    for ex in train_data:
        ...
        if fn not in fnum_records and ex.get("union_name"):
            ...
            # CWA 5-digit locals: extract leading digit as prefix
            if (
                ex["union_name"] == "COMMUNICATIONS WORKERS AFL-CIO"
                and isinstance(desig_num, (int, float))
                and desig_num >= 10000
                and prefix in (-100, 0, None)
            ):
                prefix = int(desig_num) // 10000
            rec = {
