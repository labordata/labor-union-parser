"""ArcFace F_num Classifier with FastText + Bloom + MLP Prototype Factoring.

Same encoder as arcface_fasttext_bloom_spike.py (FastText words + bloom numbers).
Prototype factoring changed from additive sum to MLP over concatenated field
embeddings + per-fnum residual:

    prototype = MLP([W_union; W_desig_name; W_desig_num; W_prefix; W_suffix]) + W_fnum

This allows the model to learn field interactions (e.g., specific union+number
combinations) that additive factoring cannot express.
"""
...
class MLPFactoredArcFaceClassifier(nn.Module):
    """ArcFace with MLP-factored prototypes: MLP([fields]) + W_fnum.

    Field embeddings are concatenated and passed through an MLP to allow
    learning field interactions. Per-fnum residual absorbs anything the
    ...
    def __init__(
        ...
        # Per-field embeddings (each produces d_model-dim vector)
        ...
        # MLP: concat of 5 fields (5 * d_model) -> d_model
        mlp_input_dim = 5 * d_model
    ...
    def _prototypes(self):
        u = self.W_union(self.field_map[:, 0])  # (n_classes, d_model)
        dn = self.W_desig_name(self.field_map[:, 1])  # (n_classes, d_model)
        pfx = self.W_prefix(self.field_map[:, 2])  # (n_classes, d_field)
        sfx = self.W_suffix(self.field_map[:, 3])  # (n_classes, d_field)
        dnum = self.bloom_embed(self.desig_bloom)  # (n_classes, d_field)

        # Concatenate and pass through MLP
        concat = torch.cat([u, dn, dnum, pfx, sfx], dim=-1)  # (n_classes, 5*d_field)
        structured = self.mlp(concat)  # (n_classes, d_model)

        return structured + self.W_fnum
...
class ArcFaceFastTextModel(nn.Module):
    def __init__(
        ...
        self.arcface = MLPFactoredArcFaceClassifier(
            d_model,
            n_classes,
