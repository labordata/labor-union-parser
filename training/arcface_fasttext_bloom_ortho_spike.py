"""ArcFace with Multi-Prototype + Orthogonality Constraint.

Based on arcface_fasttext_bloom_multiproto_spike.py. Adds an orthogonality
regularizer ||W_bloom^T @ W_union||²_F that prevents the number embedding
subspace from aligning with the union embedding subspace. This stops the
number embeddings from absorbing union-specific associations through
end-to-end training.

Also includes multi-prototype support for CWA number aliases.
"""
...
class MultiProtoArcFaceClassifier(nn.Module):
    ...
    def ortho_loss(self):
        """Orthogonality regularizer: bloom ⊥ union AND bloom ⊥ W_fnum.

        Penalizes alignment between the number embedding subspace and
        both the union embedding subspace and the per-fnum residuals.

        Uses trace trick to avoid materializing huge cross-correlation matrices:
        ||A @ B^T||²_F = trace(B @ A^T @ A @ B^T) = (B @ G_A * B).sum()
        where G_A = A^T @ A is (d, d).
        """
        W_b = F.normalize(self.bloom_embed.embed.weight, dim=1)  # (4096, d)
        G_b = W_b.T @ W_b  # (d, d) — gram matrix of bloom

        # bloom ⊥ union
        W_u = F.normalize(self.W_union.weight[1:], dim=1)  # (n_unions, d)
        loss_u = (W_u @ G_b * W_u).sum() / W_u.shape[0]

        # bloom ⊥ W_fnum
        W_f = F.normalize(self.W_fnum, dim=1)  # (n_classes, d)
        loss_f = (W_f @ G_b * W_f).sum() / W_f.shape[0]
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Weight for orthogonality regularizer between bloom and union embeddings",
    )
    ...
    for epoch in range(args.epochs):
        ...
        for start in range(0, n_train, args.batch_size):
            ...
            loss = loss + args.ortho_lambda * model.arcface.ortho_loss()
