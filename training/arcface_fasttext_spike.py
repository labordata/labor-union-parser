"""ArcFace F_num Classifier with FastText Encoder.

Same architecture as arcface_fnum_spike.py but replaces the token vocab
embedding with a FastText-style character n-gram hash embedding. This gives
open-vocabulary token representations without the cost of CharCNN.

The RoPE self-attention encoder, masked mean-pooling, and factored ArcFace
head are unchanged from the baseline.
"""
...
class FastTextRoPEEncoder(nn.Module):
    ...
    def forward(self, token_ids, ngram_ids, ngram_counts, is_num, lengths):
        ...
        x = self.token_embed(token_ids, ngram_ids, ngram_counts) + self.num_flag(
            is_num.unsqueeze(-1)
        )
...
class ArcFaceFastTextModel(nn.Module):
    ...
    def encode(self, token_ids, ngram_ids, ngram_counts, is_num, lengths):
        """Encode to L2-normalized embeddings via masked mean-pool."""
        h = self.encoder(token_ids, ngram_ids, ngram_counts, is_num, lengths)
    ...
    def forward(
        self, token_ids, ngram_ids, ngram_counts, is_num, lengths, targets=None
    ):
        embeddings = self.encode(token_ids, ngram_ids, ngram_counts, is_num, lengths)
...
def evaluate(model, data, fnum_freq, device, batch_size=512):
    ...
    with torch.no_grad():
        for start in range(0, len(data), batch_size):
            ...
            token_ids, ngram_ids, ngram_counts, is_num_t, lengths, targets = (
                collate_batch(batch, device)
            )
            logits, _ = model(token_ids, ngram_ids, ngram_counts, is_num_t, lengths)
