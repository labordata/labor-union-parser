"""ArcFace F_num Classifier with FastText Encoder + Suffix Bloom Hash Numbers.

Like arcface_fasttext_bloom_spike.py but numbers are represented by bloom
hashing ALL their suffixes, not just the whole number. This means 52031 and
31 share embedding components (both contain suffix "31") while 31 and 32
remain distinct. Designed for CWA-style prefix numbering where NABET-31
maps to CWA 52031.
...
MAX_SUFFIXES = 8  # max number of suffix hashes per token


def suffix_bloom_hash_ids(number_str):
    """Hash all suffixes of a number into bloom table indices.

    52031 -> suffixes [52031, 2031, 031, 31, 1]
    Each suffix gets NUM_BLOOM_HASHES hash indices.
    Returns flat list of all hash indices (up to MAX_SUFFIXES * NUM_BLOOM_HASHES).
    """
    normalized = number_str.lstrip("0") or "0"
    ids = []
    # Generate suffix hashes (longest first)
    for i in range(min(len(normalized), MAX_SUFFIXES)):
        suffix = normalized[i:]
        # Normalize suffix too (strip leading zeros)
        suffix_norm = suffix.lstrip("0") or "0"
        for seed in range(NUM_BLOOM_HASHES):
            h = hashlib.md5(f"{seed}:{suffix_norm}".encode()).hexdigest()
            ids.append(int(h, 16) % BLOOM_TABLE_SIZE)
    return ids


def precompute_bloom_ids(tokens, is_num):
    """Precompute suffix bloom hash IDs for all tokens.

    Each token gets MAX_SUFFIXES * NUM_BLOOM_HASHES hash slots (padded with 0).
    """
    max_hashes = MAX_SUFFIXES * NUM_BLOOM_HASHES
    all_ids = []
    all_counts = []
    for tok, is_n in zip(tokens, is_num):
        if is_n and tok:
            hashes = suffix_bloom_hash_ids(tok)
            count = len(hashes)
            padded = (hashes + [0] * max_hashes)[:max_hashes]
            all_ids.append(padded)
            all_counts.append(count)
        else:
            all_ids.append([0] * max_hashes)
            all_counts.append(0)
...
class BloomNumberEmbedding(nn.Module):
    """Simple bloom hash embedding for prototype side (3 hashes, no suffixes)."""
...
class SuffixBloomNumberEmbedding(nn.Module):
    """Suffix bloom hash embedding for encoder side — shares hash table with prototype."""

    def __init__(self, d_model, table_size=BLOOM_TABLE_SIZE):
        super().__init__()
        # Index 0 is padding (zero embedding)
        self.embed = nn.Embedding(table_size + 1, d_model, padding_idx=0)
        nn.init.normal_(self.embed.weight, std=0.01)
        self.embed.weight.data[0].zero_()

    def forward(self, bloom_ids, bloom_counts):
        shifted = bloom_ids + 1
        mask = torch.arange(
            shifted.shape[-1], device=shifted.device
        ) < bloom_counts.unsqueeze(-1)
        shifted = shifted * mask

        emb = self.embed(shifted)
        summed = emb.sum(dim=-2)
        counts_safe = bloom_counts.float().clamp(min=1).unsqueeze(-1)
        return summed / counts_safe
...
class FastTextRoPEEncoder(nn.Module):
    def __init__(
        ...
        self.bloom_embed = SuffixBloomNumberEmbedding(d_model)
    ...
    def forward(
        ...
        Args:
            ...
            bloom_ids: (B, L, max_suffix_hashes) int — suffix bloom hash indices
            bloom_counts: (B, L) int — actual hash count per number token
            is_num: (B, L) float
            lengths: (B,) int
        ...
        # Suffix bloom hash embedding for number tokens
        num_emb = self.bloom_embed(bloom_ids, bloom_counts)
...
class FactoredArcFaceClassifier(nn.Module):
    ...
    def __init__(
        ...
        # Suffix bloom for prototype side — shared table, detached to avoid slow backward
        self.bloom_embed = SuffixBloomNumberEmbedding(d_model)
        ...
        self.register_buffer("desig_bloom_counts", fnum_desig_bloom_counts)
    ...
    def _prototypes(self):
        ...
        # Suffix bloom for desig_num — detach to avoid expensive backward through 16K prototypes
        with torch.no_grad():
            dnum = self.bloom_embed(self.desig_bloom, self.desig_bloom_counts)
        # Re-attach so it participates in the forward (cosine sim) but not backward through embed
        dnum = dnum.detach()
...
class ArcFaceFastTextModel(nn.Module):
    def __init__(
        ...
        self.arcface = FactoredArcFaceClassifier(
            ...
            fnum_desig_bloom_counts=factored_info["desig_bloom_counts"],
            scale=scale,
            margin=margin,
        )
    ...
    def encode(
        ...
        h = self.encoder(
            token_ids, ngram_ids, ngram_counts, bloom_ids, bloom_counts, is_num, lengths
        )
...
def load_data(path, n_buckets, synthetic_path=None):
    ...
    for ex in raw:
        ...
        bloom_ids, bloom_counts = precompute_bloom_ids(tokens, is_num)
...
def collate_batch(batch, device):
    ...
    max_bloom = len(batch[0]["bloom_ids"][0])
    B = len(batch)
    ...
    bloom_ids = torch.zeros(B, max_len, max_bloom, dtype=torch.long)
    bloom_counts = torch.zeros(B, max_len, dtype=torch.long)
    ...
    for i, ex in enumerate(batch):
        ...
        bloom_counts[i, :L] = torch.tensor(ex["bloom_counts"][:L], dtype=torch.long)
...
def main():
    ...
    max_hashes = MAX_SUFFIXES * NUM_BLOOM_HASHES
    desig_bloom_t = torch.zeros(n_classes, max_hashes, dtype=torch.long)
    desig_bloom_counts_t = torch.zeros(n_classes, dtype=torch.long)
    ...
    for i in range(n_classes):
        ...
        if dnum not in (-100, 0, None):
            hashes = suffix_bloom_hash_ids(str(int(dnum)))
            count = min(len(hashes), max_hashes)
            for j in range(count):
                desig_bloom_t[i, j] = hashes[j]
            desig_bloom_counts_t[i] = count
    ...
    factored_info = {
        ...
        "desig_bloom_counts": desig_bloom_counts_t,
    }
    ...
    if args.save_checkpoint:
        checkpoint = {
            ...
            "desig_bloom_counts": factored_info["desig_bloom_counts"],
        }
